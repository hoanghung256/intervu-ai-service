...
class AssessmentRequest(BaseModel):
    role: str
    level: str
    # Allow single value or multiple choices for techstack/domain
...
def is_empty_request(req: AssessmentRequest):
    def _empty(v):
        ...
        if isinstance(v, (list, tuple)):
            # treat list as empty if it contains no non-empty items
...
STEP 2 — MAP SKILLS
- From {skill_reference_str}, ONLY select skills that:
...
"""
        ...
        return {
            "status": "success",
            "context_question": data.get("contextQuestion", ""),
